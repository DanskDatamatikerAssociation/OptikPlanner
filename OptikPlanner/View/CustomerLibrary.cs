...
namespace OptikPlanner.View
{
    public partial class CustomerLibrary : Form
    {
        public CustomerLibrary()
        {
            ...
            ListViewBox.Items.Add(new ListViewItem(new string[] { CustomerLibraryController.GetCustomer().CS_CPRNO, CustomerLibraryController.GetCustomer().CS_FIRSTNAME, CustomerLibraryController.GetCustomer().CS_LASTNAME}));
        ...
        private void editCustomerButton_Click(object sender, EventArgs e)
        {
            ...
            var cpr = test[0].SubItems[0].Text = CustomerLibraryController.GetCustomer().CS_CPRNO;
            var fornavn = test[0].SubItems[1].Text = CustomerLibraryController.GetCustomer().CS_FIRSTNAME;
            var efternavn = test[0].SubItems[2].Text = CustomerLibraryController.GetCustomer().CS_LASTNAME;

            window.cprBox.Text = cpr;
            window.firstNameBox.Text = fornavn;
            window.LastNameBox.Text = efternavn;
            window.adressBox.Text = CustomerLibraryController.GetCustomer().CS_ADRESS1;
            window.emailBox.Text = CustomerLibraryController.GetCustomer().CS_EMAIL;
            window.phoneBox.Text = CustomerLibraryController.GetCustomer().CS_PHONEMOBILE;
        ...
        {
            if (cprBox.Text == CustomerLibraryController.GetCustomer().CS_CPRNO)
            {
